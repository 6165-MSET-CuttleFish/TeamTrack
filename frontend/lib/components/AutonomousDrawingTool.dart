...
class _AutonPainterState extends State<AutonPainter> {
  ...
  void uploadFile(Uint8List image) async {
    ...
    Reference ref =
        storage.ref().child('${_eKey} - ${team.number} - ${scope}.png');
    UploadTask uploadTask =
        ref.putData(image, SettableMetadata(contentType: 'image/png'));
    try {
  ...
  void uploadFile2(ui.Image image) async {
    ...
    Reference ref =
        storage.ref().child('${_eKey} - ${team.number} - ${scope}.svg');
    UploadTask uploadTask =
        ref.putData(temp, SettableMetadata(contentType: 'image/svg'));
    try {
      await uploadTask.whenComplete(() {
        print('All Done');
      });
                  ...
                  children: <Widget>[
                    ...
                    GestureDetector(
                      onPanDown: (details) {
                        ...
                        final renderBox =
                            localPosition.globalToLocal(details.globalPosition);
                        if (renderBox.dx < ((xLow + xHigh) / 2)) {
                          pointsLeft++;
                        } else if (renderBox.dx > ((xLow + xHigh) / 2)) {
                          pointsRight++;
                        }
                        if (renderBox.dx >= xLow &&
                            renderBox.dy >= yLow &&
                            renderBox.dx <= xHigh &&
                            renderBox.dy <= yHigh) {
                          setState(() {
  ...
  void paint(Canvas canvas, ui.Size size) {
    ...
    for (var index = 1; index < offsets.length; ++index) {
      if (offsets[index - 1] != null && offsets[index] != null) {
        Offset a =
            Offset(offsets[index - 1].dx, offsets[index - 1].dy - magicOffset);
