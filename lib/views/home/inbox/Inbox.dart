...
class _InboxState extends State<Inbox> {
  ...
  Widget build(BuildContext context) {
    ...
    if (dataModel.inbox.isEmpty) {
      return EmptyList();
    }
    return ListView(
      children: dataModel.inbox
          .map(
            (e) => Slidable(
              actionPane: slider,
              secondaryActions: [
                            ...
                            onPressed: () async {
                              await firebaseFirestore.runTransaction(
                                (transaction) async {
                                  ...
                                  newBlocks[e.senderId] = e.senderEmail;
                        ...
                        onPressed: () async {
                          await firebaseFirestore.runTransaction(
                            (transaction) async {
                              ...
                              Map<String, dynamic> newInbox = snapshot
                                  .data()?["inbox"] as Map<String, dynamic>;
                              newInbox.removeWhere((key, value) => key == e.id);
                              Map<String, dynamic> newEvents = snapshot
